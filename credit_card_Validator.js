...
//iterate through the array of numbers, right to left, ie decedning, probably using a for i--, as you itereate double the digit,
...
const validateCred = (arr) => {
  ...
  cloneArr.reverse();
  let result = [];
  cloneArr.forEach((elem, idx) => {
    if (idx % 2 === 0) {
      result.push(elem);
    } else {
      if (elem * 2 > 9) {
        result.push(elem * 2 - 9);
      } else result.push(elem * 2);
    }
  });
  let total = result.reduce((acu, cur) => acu + cur);
  if (total % 10 === 0) {
    return true;
  } else {
    return false;
